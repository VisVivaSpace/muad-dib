...
use cspice_common::{cspice_spkgeo, hermite_spk_path, lsk_path, spk_path, CspiceKernels, CSPICE_LOCK};
...
fn debug_spk_type9_data() {
    ...
    println!("Target: {}, Center: {}, Frame: {}", spk.target_code, spk.center_code, spk.frame_code);
    ...
    match parsed {
        SpkData::Type9(type9) => {
            ...
            let (cspice_state_j2000, _lt) = cspice_spkgeo(spk.target_code, first_epoch, "J2000", spk.center_code);
            println!(
                "CSPICE J2000: pos=[{:.3}, {:.3}, {:.3}] vel=[{:.6}, {:.6}, {:.6}]",
                cspice_state_j2000[0], cspice_state_j2000[1], cspice_state_j2000[2],
                cspice_state_j2000[3], cspice_state_j2000[4], cspice_state_j2000[5]
            );

            // Query in ECLIPJ2000 frame (frame 17, which matches the segment)
            println!("\n=== CSPICE at first epoch ({}) in ECLIPJ2000 ===", first_epoch);
            let (cspice_state_eclip, _lt) = cspice_spkgeo(spk.target_code, first_epoch, "ECLIPJ2000", spk.center_code);
            println!(
                "CSPICE ECLIPJ2000: pos=[{:.3}, {:.3}, {:.3}] vel=[{:.6}, {:.6}, {:.6}]",
                cspice_state_eclip[0], cspice_state_eclip[1], cspice_state_eclip[2],
                cspice_state_eclip[3], cspice_state_eclip[4], cspice_state_eclip[5]
            );
            ...
            println!("X diff: {:.10} km", (our_state.x - cspice_state_eclip[0]).abs());
            println!("Y diff: {:.10} km", (our_state.y - cspice_state_eclip[1]).abs());
            println!("Z diff: {:.10} km", (our_state.z - cspice_state_eclip[2]).abs());
            println!("VX diff: {:.15} km/s", (our_state.vx - cspice_state_eclip[3]).abs());
            println!("VY diff: {:.15} km/s", (our_state.vy - cspice_state_eclip[4]).abs());
            println!("VZ diff: {:.15} km/s", (our_state.vz - cspice_state_eclip[5]).abs());
        }
...
fn debug_spk_type13_hermite_data() {
    ...
    println!("SPK Type: {}", spk.spk_type);
    println!("Target: {}, Center: {}, Frame: {} ({})", spk.target_code, spk.center_code, spk.frame_code, frame_name(spk.frame_code));
    println!("Initial epoch: {}", spk.initial_epoch);
    println!("Final epoch: {}", spk.final_epoch);
    ...
    match parsed {
        SpkData::Type13(type13) => {
            ...
            println!("epoch={:.3} pos=[{:.3}, {:.3}, {:.3}]", first_state.epoch, first_state.x, first_state.y, first_state.z);
            ...
            let (cspice_state, _lt) = cspice_spkgeo(spk.target_code, first_epoch, frame, spk.center_code);
            println!(
                "CSPICE {}: pos=[{:.3}, {:.3}, {:.3}]",
...
fn debug_spk_boundary_interpolation() {
    ...
    println!("First state epoch:     {:.15}", type9.states.first().unwrap().epoch);
    println!("Last state epoch:      {:.15}", type9.states.last().unwrap().epoch);
    println!("Window size: {}", type9.window_size);
    println!();
    println!("final_epoch == last state epoch? {}", spk.final_epoch == type9.states.last().unwrap().epoch);
    println!("final_epoch bits: {:016x}", spk.final_epoch.to_bits());
    println!("last state bits:  {:016x}", type9.states.last().unwrap().epoch.to_bits());
    ...
    println!("CSPICE velocity: [{:.10}, {:.10}, {:.10}]",
             cspice_state[3], cspice_state[4], cspice_state[5]);

    // Difference
    println!("\nVelocity difference:");
    for i in 0..3 {
        println!("  [{}]: diff = {:.10}", i, (muad_state.velocity[i] - cspice_state[i+3]).abs());
    }
    ...
    let muad_earlier = kernel.state_of(
        muad_dib::types::NaifId(target),
    ...
    for i in 0..3 {
        println!("  [{}]: diff = {:.10}", i, (muad_earlier.velocity[i] - cspice_earlier[i+3]).abs());
    }
    ...
    let muad_start = kernel.state_of(
        muad_dib::types::NaifId(target),
    ...
    println!("First state (idx 0) velocity: [{:.10}, {:.10}, {:.10}]",
             first_state.vx, first_state.vy, first_state.vz);
    println!("Last state (idx {}) velocity: [{:.10}, {:.10}, {:.10}]",
             type9.states.len() - 1, last_state.vx, last_state.vy, last_state.vz);
    ...
    println!("Our interpolated velocity: [{:.10}, {:.10}, {:.10}]",
             muad_state.velocity[0], muad_state.velocity[1], muad_state.velocity[2]);
    ...
    for eps_exp in [9, 6, 3] {
        ...
        println!("VX diff: {:.15}", (muad_eps.velocity[0] - cspice_eps[3]).abs());
    }
}
...
fn debug_spk_raw_segment_data() {
    ...
    if n >= 2 {
        ...
        println!("\n=== First 5 epochs (starting at index {}) ===", epochs_start);
